...
交付给客户/现场的唯一入口：一个网页完成 2D 手眼标定（头部 / 腰部 RGB 相机 → `torso_link` 外参，并存档 SDK 内参）和
3D 手安装 / TCP 标定，
...
| `hand_connections` | 18005侧的连接参数。强脑默认Modbus左126/右127；可按设备设置 `transport`，按 `left`/`right` 设置 `slave_id`、`port` 或DDS参数，不修改18089配置 |
...
18089服务需提前启动，但无需在其页面手动连接：勾选零位保持后18005会自动请求连接。
已有连接必须与目标设备、通信方式、侧别及Modbus地址匹配，并且有在线反馈，否则拒绝发送；
请用户先在18089断开不匹配的旧连接再重试，不会自动断开或抢占其他控制源。
保持期间再次检查连接，发现断连、错配或控制冲突即停止发送并记录错误。
18089原页面的左右手地址不会因本补丁自动联动，手动使用该页面时仍需填写正确ID。
机械臂接管、归位、轨迹校验及人工监护要求不变，软件停止不能代替实体急停。
3D页面各阶段和全屏选点顶部均保留机械臂控制栏：立即停止、卸力拖动、保持、结束接管。
运行/预检/暂停中只允许停止，待轨迹停止后才允许切换到人工操作；卸力拖动和结束接管须确认并扶稳手臂。
进入选点不再自动解除接管；操作员应先将手臂安置安全，再明确点击“结束接管”。该操作不清除采集或选点数据。
确认标定对象后自动进入占满浏览器视口的选点界面，可通过顶部“退出全屏”回到向导，或通过底部进入求解。
重新打开选点页面时，先按当前任务保存的 `run_dir` 恢复3D会话，再加载操作台；恢复不重置选点或求解结果。
若目录不可读或18000当前上下文不匹配，会显示错误而不是静默打开默认的空目录。
